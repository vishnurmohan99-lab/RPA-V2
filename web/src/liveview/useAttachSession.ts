...
export function useAttachSession(kind: AttachKind, id: string) {
  ...
  // Sets a password for this server session only (in-memory, never persisted) and fills it on
  // the real page for real — see credentials.ts. Only meaningful while authoring.
  const setCredential = async (signInId: string, value: string) => {
    if (kind === 'browse') await api.browseSetCredential(id, signInId, value).catch(() => {});
  };

  return { screenshot, highlight, logs, connected, click, scroll, type, setCredential };
}
